                              ...
                              @if (expandedSubProject === subProject.id) {
                                ...
                                >
                                  <div class="px-4 pb-3 border-t border-white/10">
                                    ...
                                    <!-- SubProject Details... (Simplified for brevity, matches structure) -->
...
export class ProjectsComponent implements OnInit, OnDestroy {
  ...
  private loadProjects(lang: string): void {
    this.apiService.getProjects(lang)
      .pipe(takeUntil(this.destroy$))
  ...
  toggleProject(id: string): void {
    ...
    if (this.expandedProject) {
      setTimeout(() => {
        const element = this.projectElements.find((el, index) => this.projects[index].id === id);
  ...
  toggleSubProject(id: string): void {
    ...
    if (this.expandedSubProject) {
      setTimeout(() => {
        // Logic to find subproject element is slightly complex without direct map, 
        // but scrolling to nearest might be implicit or we can refine later.
        // For now, simple toggle is sufficient.
      }, 300);
    }
  }
